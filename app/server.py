...
# ---- Robust absolute path to the model (independent of CWD) ----
...
def health():
    return jsonify({"ok": True, "model": os.path.basename(MODEL_PATH)}), 200
...
def model_output_parse():
    try:
        ...
        connectinator.logger.info('Received request on /model_output: %s', model_output)
...
def get_sign_to_text():
    translated_message = connectinator.get_transltion()  # note: method name in Connectinator has the typo
...
def process_recording():
    try:
        ...
        with open(os.path.join(HERE, 'stats.json'), 'r', encoding='utf-8') as f:
            stats = json.load(f)
        ...
        # map to labels
        with open(os.path.join(HERE, 'label_map.json'), 'r', encoding='utf-8') as f:
            lm = json.load(f)
        ...
        # store last model output for polling endpoints
...
def get_phrase():
    return connectinator.front_end_translation_variable


@app.route('/api/get_top_predictions', methods=["GET"])
def get_top_predictions():
    return jsonify({"top_5": connectinator.get_top_predictions()}), 200


@app.route('/api/get_top_1', methods=["GET"])
def get_top_1():
    return jsonify({"top_1": connectinator.get_top_1_prediction()}), 200


if __name__ == "__main__":
    # Use 5001 for API so Vite can keep 5173; add a Vite proxy from /api to 5001.
